...
class action {
    ...
    load = async () => {
        ...
        if (this.component.props.maintenanceId || this.component.props.maintenanceId == 0) {
            response = await this.webapi.Maintenance.findById(this.component.props.maintenanceId)
            payload.Maintenance = response
        }
    ...
    save = async () => {
        ...
        if (form.id || form.id == 0) {
            const response = await this.webapi.Maintenance.update(form)
            if (response) {
                this.metaAction.toast('success', '保存人员成功')
                this.injections.reduce('setMaintenance', response)
            }

        } else {
            const response = await this.webapi.Maintenance.create(form)
            if (response) {
                this.metaAction.toast('success', '保存人员成功')
                this.injections.reduce('setMaintenance', response)
            }
    ...
    add = () => {
        this.injections.reduce('setMaintenance', {
            id: undefined,
            name: '',
            sex: '0',
            birthday: `1980-01-01`,
    ...
    prev = async () => {
        ...
        const response = await this.webapi.Maintenance.prev(id)
        if (response) {
            this.injections.reduce('setMaintenance', response)
        }
    }
    ...
    next = async () => {
        const id = this.metaAction.gf('data.form.id')
        const response = await this.webapi.Maintenance.next(id)
        if (response) {
            this.injections.reduce('setMaintenance', response)
        }
    }
