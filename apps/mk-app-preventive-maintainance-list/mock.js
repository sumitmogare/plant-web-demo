...
function initMaintainanceList() {
    if (!mockData.MaintainanceList || !mockData.MaintainanceList.length==0 ) {
        mockData.MaintainanceList = []
        for (let i = 0; i < 200; i++) {
            mockData.MaintainanceList.push({
                id: i,
                name: 'username ' + (i + 1),
                sex: i % 2 + '',
...
fetch.mock('/v1/MaintainanceList/query', (option) => {
    
    const { pagination, filter } = option
    if (!option.filters){
        initMaintainanceList()
        initDepartments()
    }
    var data = mockData.MaintainanceList
    if (filter) {
        if (filter.name)
...
fetch.mock('/v1/MaintainanceList/deleteRec',(option)=>{
    console.log(option);
    
    //initMaintainanceList();
    console.log(mockData.MaintainanceList);
    let index = mockData.MaintainanceList.findIndex(o => o.id == option.id)
        
        if (index || index === 0)
            mockData.MaintainanceList.splice(index, 1)
