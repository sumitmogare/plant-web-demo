...
class action {
    ...
    load = async (pagination, filter = {}) => {
        const response = await this.webapi.jobplan.query({ pagination, filter })
    ...
    editRow = async (...args) => {
        ...
        if(!this.config.apps['mk-app-jobplan-card']){
            throw '依赖mk-app-jobplan-card app,请使用mk clone mk-app-jobplan-card命令添加'
        }
        ...
        const ret = await this.metaAction.modal('show', {
            ...
            okText: 'Save',
            cancelText: 'Cancel',
            children: this.metaAction.loadApp('mk-app-jobplan-card?from=list', {
