...
class action {
    ...
    load = async () => {
        const response = await this.webapi.assetTree.findById(this.component.props.typeId)
    ...
    save = async () => {
        ...
        if (form.id) {
            const response = await this.webapi.assetTree.update(form)
            this.metaAction.toast('success', 'Successfully modified')
            return response
        }
        else {
            const response = await this.webapi.assetTree.create(form)
