...
fetch.mock('/v1/assetTree/create', (option) => {
    const parent = findNode(option.parentId, mockData.assetTree)
    var newId = -1
    if (!parent.children) {
        parent.children = []
...
fetch.mock('/v1/assetTree/update', (option) => {
    const node = findNode(option.id, mockData.assetTree)
    node.code = option.code
    node.name = option.name
    return { result: true, value: node }
})

fetch.mock('/v1/assetTree/findById', (id) => {
    const node = findNode(id, mockData.assetTree)
    return { result: true, value: node }
})